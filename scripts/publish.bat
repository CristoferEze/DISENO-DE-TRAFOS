...
REM --- VALIDACION ADICIONAL: Comprobar que el token funcione y tenga permisos ---
echo [1.5/3] Validando token de GitHub...
REM gh CLI detecta GITHUB_TOKEN desde el entorno de este proceso
gh api user --silent >nul 2>&1
IF ERRORLEVEL 1 GOTO AUTH_FAIL
echo    Token valido y autenticacion verificada.
echo.

GOTO VALIDATION_DONE

:AUTH_FAIL
echo(*** ERROR: La autenticacion con GitHub falló (HTTP 401 o token inválido).
echo(El token en '.env' puede ser inválido, expirado o no tener los permisos necesarios (scope 'repo').
echo(
echo Sugerencias:
echo  - Crea un Personal Access Token con el scope 'repo' (o 'public_repo' para repositorios publicos) y actualiza '.env'.
echo  - Si prefieres usar autenticacion interactiva, borra o deshabilita la variable GITHUB_TOKEN del entorno antes y ejecuta: gh auth login
echo  - Comprueba que no haya espacios ni comillas en la variable GITHUB_TOKEN dentro de '.env'.
exit /b 1
...
IF %ERRORLEVEL% NEQ 0 (
    REM Si gh devolvió 403, ofrecer mensaje más explicativo
    echo Verificando codigo de error de gh...
    REM Intentar recrear la misma llamada con salida detallada para capturar el error
    gh release create %VERSION_TAG% "%SETUP_FILE%" --repo %REPO% --title "Release %VERSION_TAG%" --notes "Lanzamiento de la version %VERSION_TAG%." 2>gh_release_error.txt
    findstr /C:"HTTP 403" gh_release_error.txt >nul 2>&1
    IF ERRORLEVEL 0 (
        echo(*** ERROR: Permiso denegado (HTTP 403). El token no tiene permisos para crear releases en este repositorio. ***
        echo(Posibles causas:
        echo( - Estás usando un token con permisos limitados (p. ej. token fine-grained sin acceso al repositorio o token con scopes insuficientes).
        echo( - Si usaste un token "classic", asegúrate de que tenga el scope 'repo'.
        echo( - Si usaste un token "fine-grained", asegúrate de que incluya acceso al repositorio y la acción para crear releases.
        echo(
        echo Soluciones:
        echo( - Regenera un Personal Access Token con el scope 'repo' (o concede acceso al repositorio en un token fine-grained).
        echo( - Alternativamente, autentícate con 'gh auth login' (interactivo) y vuelve a ejecutar este script.
        del gh_release_error.txt >nul 2>&1
        exit /b 1
    )
    del gh_release_error.txt >nul 2>&1
